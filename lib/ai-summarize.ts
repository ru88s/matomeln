...
- その上で、内容に応じて追加装飾：
  * レス1（スレ立て） → "red"（赤色、自動設定）
  * 面白いツッコミ・印象的な発言 → "red"（赤色）
  * 補足説明・冷静な指摘 → "blue"（青色）
  * 為になる・参考になる・勉強になる・役立つコメント → "green"（緑色）
  * アンカー先として追加されたレス → 色は自動設定されます
  * 最後のレス（落ちコメント） → "red"（自動設定）
  * **重要でないレス・補足的なレス・淡々とした説明のみのレス → null（色なし）**

【色を付けるかどうかの判断基準 - 重要！】
- **色は「非常に面白い」「非常に印象的」「非常に重要」なレスにのみ付けてください**
- **デフォルトはnull（色なし）です。迷ったらnullにしてください**
- 以下のようなレスには絶対に色を付けず、null を設定してください：
  * 単なる補足説明や淡々とした情報提供のみのレス
  * 会話の流れを繋ぐだけの重要度が低いレス
  * 特に印象に残らない平凡なレス
  * 前のレスと同じ色になってしまう場合（最優先！）
- **色付きのレスは全体の30-40%程度に抑えてください（色なしのレスが過半数でOK）**
- **連続した色を避けるため、nullを積極的に活用してください**

【サイズブーストルール - 慎重に使用すること】
- **"size_boost": "large"（サイズ大）は非常に特別なレスのみに使用**
- 使用例：
  * スレ全体のクライマックス・オチとなる決定的な発言
  * 圧倒的なインパクトがあり、誰もが驚くような発言
  * スレの結論・まとめとなる重要な発言
- **原則として全レスの5-10%以下（0-2件程度）に限定してください**
- 迷ったら"size_boost": nullにしてください（デフォルトはサイズ中）

【色の使用ルール - 最重要！必ず守ること】
- **使用できる色は「red」「blue」「green」「null」の4種類のみです**

**🚨 連続した色の禁止ルール（最優先事項）🚨**
- **絶対に連続するレス番号に同じ色を割り当てないでください**
- **これは読みやすさに直結する最も重要なルールです**
- **連続した色になりそうな場合は、迷わずnullを選んでください**
- 例：レス3が赤色の場合、レス5は青色・緑色・nullのいずれかにする（赤色は絶対禁止）
- 例：レス10が青色の場合、レス12は赤色・緑色・nullのいずれかにする（青色は絶対禁止）

**色付けの優先順位（重要）**
1. **まず全てのレスの色をnullに設定する**
2. **その中から本当に重要・印象的なレスにのみ色を付ける**
3. **色を付ける際は、必ず前のレスと異なる色を選ぶ**
4. **迷ったらnullのままにする（色なしのレスが50%以上あっても問題ありません）**

**最終チェック（必須）**
- 色付けが完了したら、selected_postsを番号順に並べて以下を確認：
  1. 連続した色がないか？
  2. もし連続していたら、後ろのレスをnullまたは別の色に変更する
- **このチェックを必ず行ってください。これが最も重要です。**
...
export function enhanceAIResponse(
  ...
  for (const post of selectedPosts) {
    if (post.decorations.color && post.decorations.color === prevColor) {
      post.decorations.color = null; // 連続した色はnullに
    }
