/**
 * タグサムネイル更新APIプロキシ（Cloudflare Functions版）
 * スレメモくんのタグDBにサムネイルURLを保存
 */
...
export async function onRequest(context: { request: Request }) {
  ...
  // POSTリクエスト：タグにサムネイルURLを保存
  if (request.method === 'POST') {
    try {
      const body = await request.json() as { tagId?: string; thumbnailUrl?: string };
      const { tagId, thumbnailUrl } = body;

      if (!tagId || !thumbnailUrl) {
        return new Response(
          JSON.stringify({ error: 'tagId and thumbnailUrl are required' }),
          {
            status: 400,
            headers: {
      ...
      const response = await fetch(
        `${THREAD_MEMO_BASE_URL}/api/tags/thumbnail`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
