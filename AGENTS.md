...
- 生活系サイト（鬼女まとめ、婚外ちゃんねる、基地沢、奥様、アンビリ、喪女リカ、怒り新党、おにひめちゃん、はーとらいふ、キチママちゃんねる、最強の鬼女様、鬼女梅）は同時投稿の初期選択・自動選択に含める。ガルちゃん系（girlschannel.net）のみ許可し、5ch/Talk/open2ch/2ch.sc/まとめ記事由来は生活・雑談っぽく見えても生活系サイトには投稿しない。ガルちゃん系でもニュース系・政治系は必ずスキップする。
...
- サムネの基本テイストは、まとめブログ向けの独自キャラクターを使った、繊細な線・透明感のある光・鮮やかな色・奥行きのある背景を備えた高品質な現代アニメのキービジュアル調にする。極端なチビ体型、硬い3Dフィギュア調、特定の商品・既存キャラクター・実在人物の顔には寄せない。
- サムネのテイストは記事ジャンルに応じて `anime_key_visual`、`product_photo`、`editorial_photo`、`mascot` から選ぶ。生活・恋愛・雑談・面白・ゲーム・漫画・芸能・スポーツはアニメ、食べ物・旅行・風景・商品は商品写真風、事件・政治・災害は人物の顔を出さない象徴的な報道写真風、判定不能はマスコットを基本にする。
- 実在人物・芸能人・スポーツ選手を写真風に生成しない。事件・政治・災害の写真風サムネは実際の現場写真と誤認されない象徴表現にし、人物の顔や特定個人を出さない。写真系ではキャラクター参考画像を画像生成APIへ送らない。
- ローカルLLMが選んだテイストは固定ルールで検証し、許可範囲外ならルール側のテイストへ矯正する。再利用ライブラリにはテイストを保存し、記事側の期待テイストと一致する画像だけ再利用する。テイスト情報を持たない旧タグキャッシュは写真系記事では使用しない。
