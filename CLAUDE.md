...
### プロンプト設計（重要）
- **プロンプトは簡潔に保つ**（トークン削減のため）
- 冗長なルール説明や例示は避ける
- レス本文のフォーマット: `番号[主]: 本文`（改行区切り）
- **すたくらくんと同じHaikuモデル・同じプロンプト形式を使用**

### 大量レス対応（1000レス超）
- **レス本文の自動切り詰め**:
  - 1000レス超: 1レスあたり100文字に制限
  - 500レス超: 1レスあたり200文字に制限
  - 100レス以下: 制限なし（1000文字まで）
- 切り詰めはAI選択時のみ、まとめ記事には元の全文を使用
- **孤立サロゲート除去**: コメント本文の不正Unicode文字を除去
  - `sanitizeText`関数でプロンプト全体をサニタイズ
  - JSONエンコードエラー「no low surrogate in string」を防止
...
- スパム的な短文コメント（15文字未満）
- 荒らしや煽りのみのレス
- **キーワードスパム**（同じ単語を大量に繰り返すレス）
  - `isKeywordSpam`関数で自動検出
  - ユニーク率30%未満、または繰り返し率50%超でスパム判定
...
### エラーハンドリング
- トークン制限エラー時: 「レスが多すぎます（○件）。このスレッドはスキップされます。」
- 一括処理ではエラーが出てもスキップして次のスレッドに進む
- APIエラーはコンソールにログ出力
