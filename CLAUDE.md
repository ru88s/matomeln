...
- 1000レス超のスレッドでも処理できるよう、プロンプトは25行程度に圧縮
- 冗長なルール説明や例示は避ける
- レス本文のフォーマットも簡潔に: `番号[主]: 本文`（改行区切り）
- **すたくらくんと同じHaikuモデルを使用**しているため、プロンプトの長さがトークン消費の主要因
...
### トークン制限対策
- プロンプト生成時にトークン数を推定（日本語1文字≒2トークン）
- 180,000トークンを超える場合はエラーを返す
- エラーメッセージ: 「レスが多すぎます（○件、推定○kトークン）。500件以下のスレッドを選んでください。」
